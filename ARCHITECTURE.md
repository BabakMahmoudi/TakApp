...
TakApp is a **non-custodial** mobile-first PWA that lets users pay for coffee with the project's own **TAK** token on the Stellar blockchain. TAK is a **SEP-41 Soroban token** — contract `CBI3WR5NQZUQ5PAPV4TBCOFMJ3MOJVZVMH5CKCGVOP63YV2SPFZN3Z7C`, 7 decimals. Users hold and control their own Stellar keys; the backend never stores or sees secret keys and can never move user funds.
...
flowchart LR
  ...
  Next --> SOROBAN[Soroban RPC<br/>SEP-41 TAK]
  Next --> SEP10[SEP-10 challenge/verify]
  Next --> Fund[Funding account<br/>createAccount (XLM)]
  WebWorker[Signing Web Worker<br/>in browser] --> Next
  Next -.->|/api/stellar/* proxy| HRZ
  Next -.->|/api/stellar/* proxy| SOROBAN
...
- **tRPC server**: All communication (auth, wallet, coffee-shop, admin, users, payments, orders, push) goes through tRPC procedures hosted in the Worker. `users` exposes profile read/edit and a display-name + public-key search; `payments.record` indexes client-submitted payments idempotently by tx hash; `orders` places/reads orders (server-authoritative totals) and `push` registers Web Push subscriptions.
...
5. The server funds the new account from the funding account (`createAccount`, XLM). SEP-41 tokens need no trustline, so signup proceeds straight to login.
...
### Coffee ordering (home → order → ready → notify)

1. From Home, "Buy Coffee" requests the browser location and picks the nearest active shop within 50 m (auto-select → its order page); otherwise the user lands on the shop list (`/buy`), which searches and sorts by distance (when location is known) or `previousOrderCount` desc.
2. On `/order/[shopId]` the user steps menu quantities (client-side total via `mulStroops`) and taps Pay. The client signs **one** TAK transfer of the summed cart total to the owner's public key (existing Web Worker path) and submits it to Horizon.
3. `orders.place` validates the shop/owner, reloads menu prices server-side, recomputes the total (rejecting `CONFLICT` if it differs from the signed amount, i.e. the menu changed mid-order), and persists one `orders` row + `order_items` snapshots + a linked `payments` row in a single batch, idempotent on `tx_hash`. It then fires a best-effort Web Push to the owner ("New order").
4. The owner sees the order on `/owner` (polled ~15 s), taps "Mark ready" → `orders.markReady` (owner/admin only) sets `status='ready'` and pushes "Your order is ready" to the customer.
5. The customer tracks status on `/orders` ("My orders") and via the push notification.
...
- `/buy` — buy coffee: shops list with search, Quote of the Day, optional client-side proximity sort via geolocation, and fallback sort by `previousOrderCount`; each shop is a "select" row that opens its order page.
